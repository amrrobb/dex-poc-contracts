...
contract DexRouter {
    ...
    function _swap(uint256[] memory amounts, address[] memory path, address _to) internal {
        for (uint256 i; i < path.length - 1; i++) {
            ...
            DexPair(pairFor(input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    ...
    function pairFor(address tokenA, address tokenB) internal view returns (address pair) {
        ...
        pair = DexFactory(factory).getPair(token0, token1);
    }

    function getReserves(address tokenA, address tokenB) internal view returns (uint256 reserveA, uint256 reserveB) {
        (address token0,) = sortTokens(tokenA, tokenB);
        (uint256 reserve0, uint256 reserve1,) = DexPair(pairFor(tokenA, tokenB)).getReserves();
        ...
        returns (uint256[] memory amounts)
    {
        require(path[0] == WETH, "DEX: INVALID_PATH");
        amounts = getAmountsIn(amountOut, path);
        require(amounts[0] <= msg.value, "DEX: EXCESSIVE_INPUT_AMOUNT");
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(pairFor(path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // Refund dust ETH, if any
